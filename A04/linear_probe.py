...
def get_transforms(image_size: int, train: bool):
    """Build split-specific EuroSAT transforms."""
    if train:
...
def subset_with_fraction(dataset, frac: float, seed: int):
    """Sample a deterministic subset for a label fraction."""
...
def train_supervised_fraction(
    ...
    """Train supervised ResNet-18 baseline on the same label fraction."""
    subset = subset_with_fraction(train_ds, frac, args.seed)
    loader = DataLoader(
        subset,
        batch_size=args.batch_size,
...
def main(args: argparse.Namespace) -> None:
    ...
    for i, frac in enumerate(fracs):
        sup_acc, sup_state = train_supervised_fraction(
            frac, train_ds, test_loader, args, device
        )
        sup_accs.append(sup_acc)
        if abs(frac - 0.1) < 1e-8:
            torch.save(
                {
        ...
        print(
            f"Fraction {frac:.2f} | SimCLR probe acc: {simclr_accs[i]:.4f} "
